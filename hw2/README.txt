...
- The purpose of this homework is to familarize the class with managing computations through scription and automation.
- Here we will automate computations to find e roots of a function f(x), 
- i.e., approximate  solutions  to  the  equationf(x)  =  0.   
- We  will  consider  three  different  functions:f(x)  =x, f(x) =x^2, and f(x) = sin(x) + cos(x)^2.  We will use Newton’s method to find the roots.
- This README file contains the instructions to reproduce these results 
- and report to be pushed to lobogit including the necessary commands
